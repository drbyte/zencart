...
if (!isset($_GET['products_id']) || SHOW_PRODUCT_INFO_COLUMNS_XSELL_PRODUCTS < 1) return;
...
if ($common_sort_order) {
    $xsell_query_sql .= " order by p.products_xsell_sort_order asc limit " . MAX_DISPLAY_XSELL;
} else {
...
if ($num_products_xsell < MIN_DISPLAY_XSELL || $num_products_xsell == 0) return;
?>
    <!-- xsell_products //-->
<?php
$row               = 0;
...
while (!$xsell_query->EOF) {
    ...
    if ($common_sort_order) {
        $xsell_image = zen_image_OLD(DIR_WS_IMAGES . $xsell_query->fields['products_image'], $xsell_query->fields['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT);
    } else {
    ...
    $xsell_query_text              = '<a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']),
            'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . $xsell_image . '</a><br /><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']),
            'products_id=' . $xsell_query->fields['products_id']) . '">' . $xsell_query->fields['products_name'] . '</a>' . (XSELL_DISPLAY_PRICE == 'true' ? '<br />' . zen_get_products_display_price($xsell_query->fields['products_id']) : '');
