...
switch ($_GET['action']) {
    case 'update_cross' :
        if ($_POST['product']) {
            foreach ($_POST['product'] as $temp_prod) {
                if (!is_numeric($temp_prod)) continue;
                $db->execute('delete from ' . TABLE_PRODUCTS_XSELL . ' where xsell_id = "' . (int)$temp_prod . '" and products_id = "' . (int)$_GET['add_related_product_ID'] . '"');
            }
        }
        ...
        if ($_POST['cross']) {
            foreach ($_POST['cross'] as $temp) {
                $sort++;
                $insert_array = [
                    'products_id' => $_GET['add_related_product_ID'],
                    'xsell_id'    => $temp,
                    'sort_order'  => $sort,
                ];
    ...
    case 'update_sort' :
        foreach ($_POST as $key => $value) {
            ...
            $db->execute('update ' . TABLE_PRODUCTS_XSELL . ' set sort_order = ' . (int)$value . ' where xsell_id = ' . (int)$key);
        }
...
if ($_GET['add_related_product_ID'] == '') {
    ...
    <table border="0" cellspacing="1" cellpadding="2" bgcolor="#999999" align="center">
            ...
            <td colspan="6">
                <table border="0" width="100%" cellspacing="0" cellpadding="2" class="infoBoxContent">
                    <tr>
                        ...
                        <td class="smallText" align="right"><?php echo $products_split->display_links($products_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page'],
                                zen_get_all_get_params(['page', 'info', 'x', 'y', 'cID', 'action'])); ?></td>
                    </tr>
                </table>
            </td>
        </tr>
        ...
        while (!$products_query->EOF) {
            ...
            ?>
            <tr onMouseOver="cOn(this); this.style.cursor='pointer'; this.style.cursor='hand';" onMouseOut="cOut(this);" bgcolor='#DFE4F4'
                onClick=document.location.href="<?php echo zen_href_link(FILENAME_XSELL, 'add_related_product_ID=' . $products['products_id'], 'NONSSL'); ?>">
                ...
                <td class="dataTableContent" valign="top">
                    <table border="0" width="100%" cellspacing="0" cellpadding="0">
                        ...
                        if ($i <= 0) {
                            ?>
                            <tr>
                                <td class="dataTableContent">&nbsp;No cross sell products&nbsp;</td>
                            </tr>
                            <?php
                        } else {
                            ?>
                ...
                <td class="dataTableContent" valign="top">&nbsp;<a
                            href="<?php echo zen_href_link(FILENAME_XSELL, zen_get_all_get_params(['action']) . 'add_related_product_ID=' . $products['products_id'], 'NONSSL'); ?>"><?php echo TEXT_EDIT_SELLS; ?></a>&nbsp;
                </td>
                <td class="dataTableContent" valign="top" align="center">&nbsp;<?php echo(($i > 0) ? '<a href="' . zen_href_link(FILENAME_XSELL,
                            zen_get_all_get_params(['action']) . 'sort=1&add_related_product_ID=' . $products['products_id'], 'NONSSL') . '">' . TEXT_SORT . '</a>&nbsp;' : '--') ?></td>
            </tr>
            <?php
...
//$products_name = mysqli_fetch_array($products_name_query);
    $products_name = $products_name_query->fields;  //['products_name'];
    ...
    <table border="0" cellspacing="0" cellpadding="0" bgcolor="#999999" align="center">
        ...
        <tr>
            <td><?php echo zen_draw_form('update_cross', FILENAME_XSELL, zen_get_all_get_params(['action']) . 'action=update_cross', 'post'); ?>
                <table cellpadding="1" cellspacing="1" border="0">
                            ...
                            <table cellpadding="3" cellspacing="0" border="0" width="100%">
                                <tr class="dataTableHeadingRow">
                                    <td valign="top" align="center" colspan="2"><span
                                                class="pageHeading"><?php echo TEXT_SETTING_SELLS . ': ' . $products_name['products_name'] . ' (' . TEXT_MODEL . ': ' . $products_name['products_model'] . ') (' . TEXT_PRODUCT_ID . ': ' . $_GET['add_related_product_ID'] . ')'; ?></span>
                                    </td>
                                </tr>
                                <tr class="dataTableHeadingRow">
                                    <td align="center" width="95%"><?php echo((file_exists(DIR_FS_CATALOG_IMAGES . $products['products_image'])) ? zen_image(DIR_WS_CATALOG_IMAGES . $products_name['products_image'], '',
                                            '', MEDIUM_IMAGE_HEIGHT) : DIR_WS_CATALOG_IMAGES . $products_name['products_image']); ?></td>
                                    <td align="right" width="5%" valign="bottom"><?php echo zen_image_submit('button_update.gif') . '<br><br><a href="' . zen_href_link(FILENAME_XSELL,
                                                'men_id=catalog') . '">' . zen_image_button('button_cancel.gif') . '<br><br>' . zen_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    ...
                    while (!$products_query->EOF) {
                        ...
                        // $xsold_count = $xsold_query->fields ;
                        ...
                        <tr bgcolor='#DFE4F4'>
                            ...
                            <td class="dataTableContent" align="center">&nbsp;<?php echo((is_file(DIR_FS_CATALOG_IMAGES . $products['products_image'])) ? zen_image(DIR_WS_CATALOG_IMAGES . $products['products_image'], '',
                                    '', SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) : '<br>No Image<br>'); ?>&nbsp;
                            </td>
                            <td class="dataTableContent">&nbsp;<?php echo zen_draw_hidden_field('product[]', $products['products_id']) . zen_draw_checkbox_field('cross[]', $products['products_id'],
                                        (($xsold_count > 0) ? true : false), '', ' onMouseOver="this.style.cursor=\'hand\'"'); ?>&nbsp;<label onMouseOver="this.style.cursor='hand'"><?php echo TEXT_CROSS_SELL; ?></label>&nbsp;
                            </td>
...
//  $products_name = mysqli_fetch_array($products_name_query);
    ...
    <table border="0" cellspacing="0" cellpadding="0" bgcolor="#999999" align="center">
        ...
        $products_split        = new splitPageResults($_GET['page'], MAX_DISPLAY_SEARCH_RESULTS, $products_query_raw, $products_query_numrows);
        $sort_order_drop_array = [];
        for ($i = 1; $i <= $products_query_numrows; $i++) {
            $sort_order_drop_array[] = ['id' => $i, 'text' => $i];
        }
        $products_query = $db->execute($products_query_raw);
        ?>
        <tr>
        ...
        <tr>
            <td><?php echo zen_draw_form('update_sort', FILENAME_XSELL, zen_get_all_get_params(['action']) . 'action=update_sort', 'post'); ?>
                <table cellpadding="1" cellspacing="1" border="0">
                    <tr>
                        <td colspan="6">
                            <table cellpadding="3" cellspacing="0" border="0" width="100%">
                                <tr class="dataTableHeadingRow">
                                    <td valign="top" align="center" colspan="2"><span
                                                class="pageHeading"><?php echo 'Setting cross-sells for: ' . $products_name['products_name'] . ' (Model: ' . $products_name['products_model'] . ') (Product ID: ' . $_GET['add_related_product_ID'] . ')'; ?></span>
                                    </td>
                                </tr>
                                <tr class="dataTableHeadingRow">
                                    ...
                                    <td align="right" valign="bottom" width="5%"><?php echo zen_image_submit('button_update.gif') . '<br><br><a href="' . zen_href_link(FILENAME_XSELL,
                                                'men_id=catalog') . '">' . zen_image_button('button_cancel.gif') . '</a>'; ?></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    ...
                    while (!$products_query->EOF) {
                        ...
                        <tr bgcolor='#DFE4F4'>
                            ...
                            <td class="dataTableContent" align="center">&nbsp;<?php echo $products['products_model']; ?>&nbsp;</td>
                            <td class="dataTableContent" align="center">&nbsp;<?php echo((is_file(DIR_FS_CATALOG_IMAGES . $products['products_image'])) ? zen_image(DIR_WS_CATALOG_IMAGES . $products['products_image'], '',
                                    '', SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) : '<br>' . TEXT_NO_IMAGE . '<br>'); ?>&nbsp;
                            </td>
