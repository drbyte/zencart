 ...
 * @version $Id: DrByte 2024 Mar 08 Modified in v2.0.0-rc1 $
 */
...
$heading = [];
$heading[] = ['text' => '<h4>' . TEXT_INFO_HEADING_COPY_TO . '</h4>'];
if (empty($pInfo->products_id)) {
    if (is_object($pInfo)) {
        $pInfo->products_id = $pID;
    } else {
        $pInfo = new objectInfo(['products_id' => $pID]);
    }
}
...
$contents[] = ['text' => zen_draw_label(TEXT_CATEGORIES, 'categories_id', 'class="control-label"') . zen_draw_pull_down_menu('categories_id', zen_get_category_tree('', '', $exclude_category), $current_category_id, 'class="form-control" id="categories_id"')];

// Javascript support uses link-only and duplicate-only classes to show/hide panels when these radio buttons are changed.
// With Link being the default checked field, all duplicate-only fields must have hiddenField initially.
$contents[] = [
...
if (zen_has_product_specials($pInfo->products_id)) {
    $contents[] = [
        'text' => '<h6>' . TEXT_COPY_SPECIALS . '</h6>' .
            '<div class="radio"><label>' . zen_draw_radio_field('copy_specials', 'copy_specials_yes', true) . TEXT_YES . '</label></div>' .
            '<div class="radio"><label>' . zen_draw_radio_field('copy_specials', 'copy_specials_no') . TEXT_NO . '</label></div>',
        'params' => 'infoBoxContent duplicate-only hiddenField',
    ];
}
...
for ($i = 0, $n = count($languages); $i < $n; $i++) {
    if (
        zen_get_product_metatag_fields($pInfo->products_id, $languages[$i]['id'], 'metatags_description')
